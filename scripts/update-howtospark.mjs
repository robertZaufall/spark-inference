...
import { assessBenchmarkFreshness, snapshotGeneratedAt } from './howtospark-freshness.mjs';
...
const freshness = assessBenchmarkFreshness({
  newestBenchmarkAt,
  previousBenchmarkAt: snapshotGeneratedAt(current || ''),
  staleAfterDays: config.maximumAgeDays
});
if (freshness.stale) {
  console.warn(
    `::warning title=HowToSpark data is stale::Newest benchmark is ${freshness.ageDays.toFixed(1)} days old; continuing with valid unchanged data.`
  );
}
