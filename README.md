...
The updater retries transient connection failures and retryable HTTP responses with bounded backoff, and limits concurrent recipe-page requests. Older but valid benchmark data is accepted with a warning; missing or malformed rows and benchmark-date regressions still stop publication. The committed snapshot means a temporary source outage cannot break the deployed site.
